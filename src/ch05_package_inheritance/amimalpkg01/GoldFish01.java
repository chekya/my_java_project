...
public class GoldFish01 extends Animal01{    // 2번 서브클래스 상속 받기 (extends Animal01 입력)
    private int gill ; //아가미 개수  //3번 변수 입력 (다른 서브 클래스들도 2번,3번 반복)
    ...
    public GoldFish01(String name, int lifespan, String habitat, int speed, int gill) {  //7번  AnimalMain에서 배열만든 후 거기 ()안 변수빨간줄에 마우스대고 creat constructor 클릭하면 서브클래스(여기)에 생성자 생긴다. 그러면 위의 ()안 입력, 아래 두줄까지 입력한다. AnimalMain 으로 간다.
        super(name,lifespan, habitat, speed);
        this.gill = gill ;
        // ( GoldFish01 완성 후 나머지 서브클래스(Lion01,Eagle01도 메인과 왔다갔다하며 만들어 준다.
    }

    public void swim(){ //헤엄치는 동작// 4번 각 서브클래스 메소드 구색 갖추기(public void swim(){} 금붕어는 헤엄치는 메소드이므로 swim 입력),
        // 아직 아랫줄은 입력하지 않는다.
